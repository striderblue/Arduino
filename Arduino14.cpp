...
// MQTT
void publishData(const myConfig &mycnf, unsigned int queueScore) {
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
  ...
  if (strcmp(rdDoc["channel_slot"], doc["channel_slot"]) == 0 && strcmp(doc["status"], "ended") == 0) {
...
boolean mqttConnect() {
  bool rc;
  rc = strlen(mqtt_broker) > 0;
  if (rc) {
    mqttClient.setServer(mqtt_broker, mqtt_port);
    mqttClient.setCallback(mqttCallback);
    mqttClient.setBufferSize(384);

    rc = mqttClient.connect(clientId, mqtt_username, mqtt_password);
    if (rc) {
      Serial.println("The client connects to the public mqtt broker");
      if (isConfigured) {
        mqttUnsubscribed();
      }
      mqttSubscribed(filename, mycnf);
...
void setup() {
  ...
  const char *prefix_client = "nt-score-";
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    if (reconnect) {
      if (millis() - lastAttempt > attempInterval) {
