...
void mqttSubscribed(const myConfig &mycnf) {
  const char *prefix_topic = "ntqueue/topic/terminal/";
  const char *serviceType = mycnf.service_type;
  const char *channelSlot = mycnf.channel_slot;

  strlcpy(subscribe_topic1, prefix_topic, sizeof(subscribe_topic1));
  strlcat(subscribe_topic1, serviceType, sizeof(subscribe_topic1));
  mqttClient.subscribe(subscribe_topic1);
  Serial.print(subscribe_topic1); Serial.println(" Subscribed!");
  
  strlcpy(subscribe_topic2, prefix_topic, sizeof(subscribe_topic2));
  strlcat(subscribe_topic2, serviceType, sizeof(subscribe_topic2));
  strlcat(subscribe_topic2, "/", sizeof(subscribe_topic2));
  strlcat(subscribe_topic2, channelSlot, sizeof(subscribe_topic2));
  mqttClient.subscribe(subscribe_topic2);
  Serial.print(subscribe_topic2); Serial.println(" Subscribed!");
  isSubscribed = true;
}

void mqttUnsubscribed() {
  mqttClient.unsubscribe(subscribe_topic1);
  mqttClient.unsubscribe(subscribe_topic2);
  isSubscribed = false;
}
